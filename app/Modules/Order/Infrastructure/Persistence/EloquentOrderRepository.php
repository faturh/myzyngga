...
class EloquentOrderRepository implements OrderRepositoryInterface
{
    ...
    public function latestByPelangganId(int $pelangganId, ?bool $finished = null): ?Transaksi
    {
        return Transaksi::query()
            ...
            ->when($finished === true, fn (Builder $query) => $query->whereHas('listPengerjaan', fn ($q) => $q->where('list_status_pengerjaan_id', 5)))
            ->when($finished === false, fn (Builder $query) => $query->whereHas('listPengerjaan', fn ($q) => $q->where('list_status_pengerjaan_id', '!=', 5)))
            ->latest('waktu')
            ->first();
    }
